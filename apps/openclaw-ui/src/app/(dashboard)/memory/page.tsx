...
export default function MemoryPage() {
  ...
  const loadFileTree = useCallback(async (workspace: string) => {
    try {
      ...
      setError("Dosya ağacı yüklenemedi");
      console.error(err);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSelectFile = useCallback(
    async (path: string) => {
      if (hasUnsavedChanges) {
        const confirmed = window.confirm("Kaydedilmemiş değişiklikler var. Vazgeçilsin mi?");
        if (!confirmed) return;
      }
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ padding: "24px 24px 16px 24px", flexShrink: 0 }}>
        ...
        <p style={{ fontFamily: "var(--font-body)", fontSize: "13px", color: "var(--text-secondary)" }}>
          Ajanların hafıza dosyalarını görüntüleyin ve düzenleyin
        </p>
      </div>
                  ...
                  {selectedPath ? (
                    ...
                    >
                      <div style={{ textAlign: "center" }}>
                        ...
                        <p style={{ fontSize: "14px" }}>Görüntülemek veya düzenlemek için bir dosya seçin</p>
                      </div>
                    </div>
                  )}
                </div>

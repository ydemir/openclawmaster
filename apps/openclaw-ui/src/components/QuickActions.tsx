...
export function QuickActions({ onActionComplete }: QuickActionsProps) {
  ...
  const handleRestartGateway = async () => {
    ...
    showNotification("success", "Ağ geçidi yeniden başlatma komutu gönderildi (örnek)");
  };
  ...
  const handleClearActivityLog = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Günlük temizlenemedi");

      showNotification("success", "Aktivite günlüğü başarıyla temizlendi");
      onActionComplete?.();
    } catch {
      showNotification("error", "Aktivite günlüğü temizlenemedi");
    } finally {
  ...
  const handleViewLogs = async () => {
    ...
    showNotification("success", "Ağ geçidi logları açılıyor... (örnek)");
  };

  const actions: ActionButton[] = [
    {
      id: "restart",
      label: "Ağ Geçidini Yeniden Başlat",
      icon: RefreshCw,
      color: "blue",
          ...
          {actions.map((action) => {
            ...
            return (
              ...
              >
                ...
                {action.placeholder && (
                  <span className="text-xs opacity-50">(örnek)</span>
                )}
              </button>
            );
          })}
